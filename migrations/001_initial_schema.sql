...
CREATE TYPE user_role AS ENUM ('free', 'pro', 'team_member', 'team_admin', 'enterprise');
CREATE TYPE document_status AS ENUM ('uploading', 'processing', 'completed', 'failed');
CREATE TYPE file_type AS ENUM ('pdf', 'docx', 'txt', 'md', 'image', 'video', 'audio', 'other');
...
CREATE TABLE users (
    ...
    storage_limit_bytes BIGINT DEFAULT 5368709120 NOT NULL, -- 5GB for free tier
...
CREATE TABLE workspaces (
    ...
    -- Settings
    storage_limit_bytes BIGINT DEFAULT 107374182400 NOT NULL, -- 100GB for teams
    storage_used_bytes BIGINT DEFAULT 0 NOT NULL,
...
CREATE TABLE documents (
    ...
    ocr_confidence DECIMAL(5,4), -- 0.0 to 1.0
    
    -- AI metadata
    language VARCHAR(10) DEFAULT 'en',
    reading_time_minutes INTEGER,
    complexity_score DECIMAL(3,2), -- Flesch-Kincaid or similar
...
CREATE TABLE transformations (
    ...
    -- Transformation type
    target_level VARCHAR(50) NOT NULL, -- 'kid', 'student', 'expert', 'phd'
    ...
    CONSTRAINT transformations_source_check CHECK (
        (source_document_id IS NOT NULL AND source_note_id IS NULL) OR
        (source_document_id IS NULL AND source_note_id IS NOT NULL)
    )
);
...
CREATE TABLE search_history (
    ...
    click_position INTEGER, -- Position of clicked result (1-indexed)
...
CREATE TABLE activity_log (
    ...
    action VARCHAR(100) NOT NULL, -- 'document.created', 'user.login', etc.
    resource_type VARCHAR(50), -- 'document', 'note', 'tag', etc.
    resource_id UUID,
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        IF NEW.workspace_id IS NOT NULL THEN
            UPDATE workspaces SET storage_used_bytes = storage_used_bytes + COALESCE(NEW.file_size_bytes, 0)
            WHERE id = NEW.workspace_id;
        ELSE
            UPDATE users SET storage_used_bytes = storage_used_bytes + COALESCE(NEW.file_size_bytes, 0)
            WHERE id = NEW.user_id;
        END IF;
    ELSIF TG_OP = 'DELETE' THEN
        -- Decrement storage
        IF OLD.workspace_id IS NOT NULL THEN
            UPDATE workspaces SET storage_used_bytes = storage_used_bytes - COALESCE(OLD.file_size_bytes, 0)
            WHERE id = OLD.workspace_id;
        ELSE
            UPDATE users SET storage_used_bytes = storage_used_bytes - COALESCE(OLD.file_size_bytes, 0)
...
SELECT 
    ...
    ROUND((u.storage_used_bytes::DECIMAL / u.storage_limit_bytes) * 100, 2) AS storage_used_percentage,
