...
import { config, qdrantClient } from '../config';
import crypto from 'crypto';
import { logEmbedding, logError } from '../utils/structuredLogger';
...
constructor() {
    this.endpoint = config.huggingface?.endpoint || 'https://api-inference.huggingface.co/models/sentence-transformers/all-MiniLM-L6-v2';
    this.apiKey = config.huggingface?.apiKey;
    logEmbedding('info', 'Embedding service initialized', {
        endpoint: this.endpoint,
        apiKeyConfigured: !!this.apiKey,
        mode: 'HuggingFace API with mock fallback'
    });
}
...
isConfigured(): boolean {
    ...
    try {
        ...
        if (response.status === 401 || response.status === 403 || response.status === 429) {
            ...
            logEmbedding('warn', 'HuggingFace API unavailable, using mock embeddings', {
                reason: 'Rate limited or auth required',
                status: response?.status
            });
        }
    } catch (error) {
        // Use mock embeddings on any error
        this.apiAvailable = false;
        this.lastApiCheck = Date.now();
        logEmbedding('warn', 'HuggingFace API error, using mock embeddings', {
...
splitIntoChunks(text: string, documentId: string, userId: string, fileType ?: string, title ?: string): DocumentChunk[] {
    ...
    logEmbedding('info', 'Starting embedding generation', {
        documentId,
        contentLength: content.length
});

const chunks = this.splitIntoChunks(content, documentId, userId, fileType, title);
logEmbedding('debug', 'Created chunks', {
    documentId,
...
logEmbedding('info', 'Generated embeddings', {
    documentId,
    totalTokens,
...
logEmbedding('info', 'Stored vectors in Qdrant', {
    documentId,
...
return { chunksProcessed: chunks.length, totalTokens };
    ...
    logEmbedding('info', 'Deleted embeddings', {
        documentId,
        collectionName: this.COLLECTION_NAME
});
    }
